...
// SetRequestContext implements miniapi.Context.
func (c *ContextImpl) SetRequestContext(ctx context.Context) {
	req := c.Origin.Request().WithContext(ctx)
	c.Origin.SetRequest(req)
}
...
// ResponseHeader implements miniapi.Context.
func (c *ContextImpl) ResponseHeader(name string) string {
	return c.Origin.Response().Header().Get(name)
}
...
// Address implements miniapi.Adapter.
...
func (a *Adapter) AddRoute(prefix string, route miniapi.Route) {
